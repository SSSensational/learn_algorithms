...
describe('sort/bucket', () => {
    test('bucket', () => {
        expect(bucket(arr1.slice())).toEqual([]);
        expect(bucket(arr2.slice())).toEqual([1]);
        expect(bucket(arr3.slice())).toEqual([1, 2]);
        expect(bucket(arr4.slice())).toEqual([1, 2, 3]);

        for (let i = 0; i < 16; i++) {
            const temp = Array.from({ length: Math.floor(Math.random() * 1234 + 1) }, () => Math.floor(Math.random() * 50));
            let shuffleTemp = temp.slice();
            shuffle(shuffleTemp);
            shuffleTemp = bucket(shuffleTemp);
            let isSorted = true;
        ...
        for (let i = 0; i < 16; i++) {
            ...
            shuffle(shuffleTemp);
            expect(bucket(shuffleTemp)).toEqual(temp);
        }
    });
});